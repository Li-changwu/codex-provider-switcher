...
wire_api = "responses"
  ...
  <script nonce="${safeNonce}">
    ...
    byId('switch').addEventListener('click',()=>post({type:'switchProfile',profileId:state.profile.id})); byId('delete').addEventListener('click',()=>post({type:'deleteProfile',profileId:state.profile.id})); byId('loginOfficial').addEventListener('click',()=>post({type:'loginOfficial',profileId:state.profile.id})); byId('sync').addEventListener('click',()=>{ byId('progress').value=0; post({type:'syncSessions',profileId:state.profile.id}); });
